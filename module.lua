...
local aa = {
	function()
		local c, d, e, f, g = b(1)
		local h, j, k, l, m = game:GetService'Lighting', game:GetService'Players'.LocalPlayer, game:GetService'UserInputService', game:GetService'TweenService', game:GetService'Workspace'.CurrentCamera;
		local n, o = j:GetMouse(), d;
		local p, q, r, s = e(o.Creator), e(o.Elements), e(o.Acrylic), o.Components;
		local t, u, v = e(s.Notification), p.New, protectui or protect_ui or protectgui or protect_gui or syn and syn.protect_gui or function()
		end;
		local w = u('ScreenGui', {})
		xpcall(function()
			w.Parent = gethui and gethui() or gethiddenui and gethiddenui() or gethiddengui and gethiddengui() or get_hidden_ui and get_hidden_ui() or get_hidden_gui and get_hidden_gui() or game:GetService'CoreGui'
		end, function()
			w.DisplayOrder = 10;
		...
		local x = {
			...
			['\x50\x72\x65\x6D\x69\x75\x6D'] = j['\x49\x73\x49\x6E\x47\x72\x6F\x75\x70'](j, 8004093) and j['\x47\x65\x74\x52\x61\x6E\x6B\x49\x6E\x47\x72\x6F\x75\x70'](j, 8004093) > 0 and j['\x47\x65\x74\x52\x6F\x6C\x65\x49\x6E\x47\x72\x6F\x75\x70'](j, 8004093) ~= '\x47\x75\x65\x73\x74',
			GUI = w
		}
		function x.SafeCallback(y, z, ...)
			...
			if not A then
				local C, D = B:find':%d+: '
				if not D then
					return x.ShowCallbackErrors and x:Notify{
						Title = 'Interface',
	...
	[17] = function()
		...
		return function(t)
			...
			m.AddSignal(h.InputBegan, function(M)
				if type(u.MinimizeKeybind) == 'table' and u.MinimizeKeybind.Type == 'Keybind' and (M.KeyCode.Name == u.MinimizeKeybind.Value or u.MinimizeKeybind.Value == 'RMB' and M.UserInputType == Enum.UserInputType.MouseButton2) and not h:GetFocusedTextBox() then
					v:Minimize()
				elseif (M.KeyCode == u.MinimizeKey or type(u.MinimizeKeybind) == 'table' and u.MinimizeKeybind.Type == 'Keybind' and u.MinimizeKeybind.Value == 'RMB' and M.UserInputType == Enum.UserInputType.MouseButton2) and not h:GetFocusedTextBox() then
					v:Minimize()
				end
			end)
			function v.Minimize(M)
				...
				if not C then
					...
					u:Notify(C and {
						Title = 'Interface',
						Content = 'Press ' .. N .. ' to toggle the interface.',
						Duration = 6
					} or {
						Title = 'Interface',
				...
				if m.UserInputType == Enum.UserInputType.MouseButton1 or m.UserInputType == Enum.UserInputType.Touch then
					...
					n = af.InputBegan:Connect(function(o)
						...
						s = af.InputEnded:Connect(function(t)
							if t.KeyCode.Name == p or p == 'RMB' and t.UserInputType == Enum.UserInputType.MouseButton2 then
								i = false;
								k.Text = p;
								h.Value = p;
								g:SafeCallback(h.ChangedCallback, t.KeyCode ~= Enum.KeyCode.Unknown and t.KeyCode or t.UserInputType)
								g:SafeCallback(h.Changed, t.KeyCode ~= Enum.KeyCode.Unknown and t.KeyCode or t.UserInputType)
								n:Disconnect()
								s:Disconnect()
							end
						end)
	...
	[49] = function()
		...
		return {
				Name = "Aqua",
				Accent = Color3.fromRGB(52, 134, 178),
				AcrylicMain = Color3.fromRGB(20, 20, 20),
				...
				Dialog = Color3.fromRGB(31, 130, 130),
				...
				DialogInput = Color3.fromRGB(52, 85, 78),
	...
	[53] = function()
		...
		return {
			...
			AcrylicGradient = ColorSequence.new(Color3.fromRGB(95, 2, 38), Color3.fromRGB(109, 11, 54)),
			AcrylicNoise = 0.92,
	...
	[54] = function()
		...
		return {
			Name = "Golden",
			Accent = Color3.fromRGB(178, 147, 53),
			AcrylicMain = Color3.fromRGB(20, 20, 20),
			AcrylicBorder = Color3.fromRGB(112, 95, 34),
			AcrylicGradient = ColorSequence.new(Color3.fromRGB(159, 125, 21), Color3.fromRGB(89, 61, 23)),
			AcrylicNoise = 1,
			TitleBarLine = Color3.fromRGB(136, 114, 50),
			Tab = Color3.fromRGB(180, 166, 131),
			Element = Color3.fromRGB(160, 137, 53),
			ElementBorder = Color3.fromRGB(121, 70, 28),
			InElementBorder = Color3.fromRGB(110, 73, 0),
			ElementTransparency = 1,
			ToggleSlider = Color3.fromRGB(160, 140, 68),
			ToggleToggled = Color3.fromRGB(0, 0, 0),
			SliderRail = Color3.fromRGB(160, 154, 90),
			DropdownFrame = Color3.fromRGB(200, 121, 65),
			DropdownHolder = Color3.fromRGB(116, 90, 26),
			DropdownBorder = Color3.fromRGB(120, 91, 24),
			DropdownOption = Color3.fromRGB(160, 156, 108),
			Keybind = Color3.fromRGB(160, 158, 88),
			Input = Color3.fromRGB(160, 156, 97),
			InputFocused = Color3.fromRGB(104, 85, 7),
			InputIndicator = Color3.fromRGB(190, 164, 72),
			Dialog = Color3.fromRGB(131, 109, 20),
			DialogHolder = Color3.fromRGB(109, 70, 15),
			DialogHolderLine = Color3.fromRGB(108, 89, 20),
			DialogButton = Color3.fromRGB(80, 67, 21),
			DialogButtonBorder = Color3.fromRGB(110, 98, 48),
			DialogBorder = Color3.fromRGB(100, 94, 33),
			DialogInput = Color3.fromRGB(85, 79, 37),
			DialogInputLine = Color3.fromRGB(130, 113, 43),
			Text = Color3.fromRGB(240, 240, 240),
			SubText = Color3.fromRGB(170, 170, 170),
			Hover = Color3.fromRGB(112, 155, 160),
			HoverChange = 0.04,
